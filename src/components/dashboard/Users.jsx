...
const Users = () => {
  ...
  const students = Array.from({ length: 15 }, (_, i) => ({
    ...
    courses: faker.number.int({ min: 1, max: 8 }),
  ...
  const instructors = Array.from({ length: 8 }, (_, i) => ({
    ...
    courses: faker.number.int({ min: 2, max: 12 }),
  ...
  const admins = Array.from({ length: 3 }, (_, i) => ({
    ...
    email: faker.internet.email(),
    avatar: faker.image.avatar(),
    role: faker.helpers.arrayElement(['Super Admin', 'Admin', 'Moderator']),
    permissions: faker.helpers.arrayElements(['Users', 'Courses', 'Analytics', 'Settings'], { min: 2, max: 4 }),
    lastActive: faker.date.recent()
  }));

  const stats = [
    { name: 'Total Students', value: '1,234', icon: UserPlus, change: '+12%', changeType: 'positive' },
    { name: 'Active Instructors', value: '45', icon: Shield, change: '+3', changeType: 'positive' },
    { name: 'New Registrations', value: '89', icon: Crown, change: '+15%', changeType: 'positive' },
    { name: 'Completion Rate', value: '78%', icon: UserPlus, change: '+5%', changeType: 'positive' }
  ];
